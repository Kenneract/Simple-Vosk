...
make sure to provide attribution, and also maybe check if its blocking-code or not

note that this is a VERY simple wrapper meant for rapid prototyping, not for any production-level code.

if uploaded, provide example code

note the requirement of the model folder (+ a download link)

have to pick a licence - just use the same one that vosk is using (or same one that discord.py is using)


Documentation.md:

Have all Speech class methods and parameters?
...
class Speech():
	...
	def __init__(self, callback=simpleCallback, partial:bool=False, model:str="model",
		...
		"""
		Generates a Speech object. Run Speech.run() to start voice recognition.
		...
		self.__verbose = False #If should print debug data (x-vector distsances, etc.)
